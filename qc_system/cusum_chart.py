...
class CUSUMChart:
    ...
    def plot(self, data: Optional[np.ndarray] = None, 
        ...
        ax1.axhline(y=self.target, color='g', linestyle='--', label='目标值')
        ax1.set_ylabel('原始数据')
        ax1.legend()
        ax1.grid(True, alpha=0.3)
        
        # 绘制上CUSUM
        x = np.arange(len(self.cusum_upper))
        ax2.plot(x, self.cusum_upper, 'b-', linewidth=2, label='上CUSUM')
        ax2.plot(x, self.control_limits['UCL_upper'], 'r--', label='UCL')
        ax2.plot(x, self.control_limits['CL'], 'g--', label='CL')
        ax2.set_ylabel('上CUSUM')
        ax2.legend()
        ax2.grid(True, alpha=0.3)
        
        # 绘制下CUSUM
        ax3.plot(x, self.cusum_lower, 'r-', linewidth=2, label='下CUSUM')
        ax3.plot(x, self.control_limits['UCL_lower'], 'r--', label='UCL')
        ax3.plot(x, self.control_limits['CL'], 'g--', label='CL')
        ax3.set_xlabel('样本序号')
        ax3.set_ylabel('下CUSUM')
        ax3.legend()
        ...
        if show_violations and hasattr(self, 'violations'):
            ...
            if violations['upper_violations']:
                ax2.scatter(violations['upper_violations'], 
                           self.cusum_upper[violations['upper_violations']], 
                           color='red', s=100, marker='o', label='超出UCL')
            if violations['lower_violations']:
                ax3.scatter(violations['lower_violations'], 
                           self.cusum_lower[violations['lower_violations']], 
                           color='red', s=100, marker='o', label='超出UCL')
